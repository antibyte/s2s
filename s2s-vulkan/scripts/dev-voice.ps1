...
Start-Process -FilePath $bin -ArgumentList @(
  ...
  "--tts", "auto",
  "--language", $(if ($env:S2S_LANGUAGE) { $env:S2S_LANGUAGE } else { "de" }),
  "--tts-language", $(if ($env:S2S_TTS_LANGUAGE) { $env:S2S_TTS_LANGUAGE } else { "de" }),
  "--llm-keep-alive", $(if ($env:S2S_LLM_KEEP_ALIVE) { $env:S2S_LLM_KEEP_ALIVE } else { "5m" }),
  "--supertonic-model-dir", "$Root\models\supertonic\onnx",
  "--supertonic-voice", $(if ($env:S2S_SUPERTONIC_VOICE) { $env:S2S_SUPERTONIC_VOICE } else { "M1" }),
